...
\numberofauthors{3}
\author{
Thomas Pellissier Tanon, Denny Vrandečić\\
   \affaddr{Google, San Francisco}\\
   \email{\{thomaspt, vrandecic\}@google.com}
\alignauthor
Sebastian Schaffert\\
   \affaddr{Google, Zurich}\\
   \email{schaffert@google.com}
\alignauthor
Thomas Steiner\\
   \affaddr{Google, Hamburg}\\
...
\begin{abstract}
Freebase was a~collaborative knowledge base created in 2007 and acquired by Google in 2010.
Wikidata is a~collaborative knowledge base created by the Wikimedia movement in 2012.
Due to the success of Wikidata, Google decided to shut down Freebase and
help with the transfer of its content to Wikidata.
In this paper, we describe some of the ongoing transfer effort and challenges,
and provide an analysis of the transfer so far.
\end{abstract}
...
That migration made no exception to the rule, and had a~number of structural challenges.
Even more challenging was the cultural difference between the two involved communities.
...
After a~short introduction to the two databases, we describe our methodology and
the metrics used to measure the migration.
In order to support the migration, we have developed a~set of open source tools
that we present in the following.
We then present the results of the migration and discuss these numbers,
as well as proposed next steps.
