...
the license-compatible part of MusicBrainz~\cite{musicbrainz}.
...
or providing of tools to assist the existing editors to allow them to be more effective. With the
...
For the challenges listed in \autoref{section:challenges-of-the-migration}
...
Only dealing with two open licenses.

\textbf{Topic mappings.}
Since both databases build on Wikipedia...

\textbf{Schema mappings.}
There has been extensive research on the topic of mapping the structures
of two databases or knowledge bases~\cite{mappingsurvey1,mappingsurvey2,mappingsurvey3}.
...
for a~one-off data flow (since Freebase is now read-only),
evaluated by human contributors (therefore the quality of the mapping 
