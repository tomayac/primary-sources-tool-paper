...
As we have seen in section \autoref{sec-dataquality} the mapped data will be human curated before the importation,
so we have chosen to merge the two mapping dumps and, for the resulting 6,000 conflicts,
decided to prioritize the Google's mapping.
To improve the resulting mapping, we have also tried to do some reconciliation
based on external databases IDs shared by Freebase and Wikidata, like
MusicBrainz,\footnote{MusicBrainz: \url{https://musicbrainz.org}}
VIAF,\footnote{VIAF: \url{http://viaf.org}} \emph{etc.}
With this technique, we were able to match 800K~external database IDs \todo{not sure if it's understandable},
creating a~mapping for 600K~topics---most of them already in the merge of Google's and Samsung's mappings.
This resulted in an additional 84K items mapped.
We also used Google's Knowledge Graph\footnote{Google Knowledge Graph introduction blog post: \url{https://googleblog.blogspot.com/2012/05/introducing-knowledge-graph-things-not.html} \todo{Better reference?}} to add 100K~further mappings
by also matching a~topic and an item if they share a~Wikipedia link \todo{Wasn't it leveraging the Knowledge Graph? -- Yes, but not sure how to rewrite this section. I have added a reference to the introduction article. We should maybe have Denny's input about it}.
...
Eventually, with these four sources \todo{Google, Samsung, MusicBrainz, VIAF, Knowledge Graph. 5 Sources? -- Google, Samsung, external IDs, Knowledge Graph}, we mapped 4.56M~items.
To the best of our knowledge, therewith we have already mapped
most of the topics that could be mapped automatically, 
which is also backed by the comparably small increases gained from the ID reconciliation approach \todo{Referring to MusicBrainz and VIAF? -- yes}
...
We have mapped the majority of topics with more than 47~facts (191K~non mapped topics \textit{vs.} 195K~mapped).
